...
BattleSystem::BattleSystem(Player& player, Enemy* enemy, Renderer& renderer,
    ...
    :player_(player),enemy_(enemy), renderer_(renderer), currentLocation_(location),
    playerAbilities_(playerAbilities), enemyAbilities_(enemyAbilities){}
...
void BattleSystem::clash(){
        if (userChoice_ == 1){
            if (player_.getStamina() > 0){
                ...
                if (player_.getShield() > 0){
                    ...
                    } else if (enemyChoice_ > 3 && enemyAbilities_[enemyChoice_ - 4]->getType() == 'd'){
                            player_.setShield(player_.getShield() - enemyAbilities_[enemyChoice_ - 4]->getFactor());
                            enemyAbilities_[enemyChoice_ - 4]->setMovesCount(enemyAbilities_[enemyChoice_ - 4]->getMovesCount() - 1);
                            if (player_.getShield() > 0){
                               renderer_.printEndlineText("Враг ударил в блок");
                            } else {
                                renderer_.printEndlineText("Враг пробил щит");
        ...
        } else if (userChoice_ >= 4 && userChoice_ <= (3+player_.getAbilitiesCount())){
            if (playerAbilities_[userChoice_-4]->getMovesCount() == playerAbilities_[userChoice_-4]->getMaxMovesCount()){
                playerAbilities_[userChoice_-4]->setMovesCount(playerAbilities_[userChoice_-4]->getMovesCount() - 1);
                renderer_.printText("Вы использовали способность ");
                renderer_.printEndlineText(playerAbilities_[userChoice_-4]->getName());
                switch (playerAbilities_[userChoice_-4]->getType()){
                    case 'd':
                        if (enemyChoice_ == 2){
                        enemy_->setIsShieldOn(1);
                        enemy_->setShield(enemy_->getShield() - playerAbilities_[userChoice_-4]->getFactor());
                        if (enemy_->getShield() > 0){
                        renderer_.printEndlineText("Вы ударили в блок");
                        }
                        else {
                        ...
                        enemy_->setHp(enemy_->getHp() - playerAbilities_[userChoice_-4]->getFactor());
                        renderer_.printEndlineText("Вы попали");
                        }
                         break;
                    case 'h':
                        if (player_.getHp() + playerAbilities_[userChoice_-4]->getFactor() > playerHp_){
                            player_.setHp(playerHp_);
                        } else {
                            player_.setHp(player_.getHp() + playerAbilities_[userChoice_-4]->getFactor());
                        }
                         break;
        ...
        } else if (enemyChoice_ >= 4 && enemyChoice_ <= (3+enemy_->getAbilitiesCount())){
                enemyAbilities_[enemyChoice_-4]->setMovesCount(enemyAbilities_[enemyChoice_-4]->getMovesCount() - 1);
                renderer_.printText("Враг использовал способность ");
                renderer_.printEndlineText(enemyAbilities_[enemyChoice_-4]->getName());
                switch (enemyAbilities_[enemyChoice_-4]->getType())
                {
                case 'd':
                    if (userChoice_ != 2 && userChoice_ != 3){
                    player_.setHp(player_.getHp() - enemyAbilities_[enemyChoice_-4]->getFactor());
                    renderer_.printEndlineText("Враг попал"); 
                    }
                    break;
                case 'h':
                    if (enemy_->getHp() + enemyAbilities_[enemyChoice_-4]->getFactor() > enemyHp_){
                        enemy_->setHp(enemyHp_);
                    } else{
                        enemy_->setHp(enemy_->getHp() + enemyAbilities_[enemyChoice_-4]->getFactor());
                    }
                    break;
                default:
                    break;
...
int BattleSystem::enemyMove(){
    ...
    while (true)
    {
        ...
        if (enemyChoice_ == 1) {
            ...
            if (enemy_->getIsDodgeOn() == 0){
                break;
            }
        } else if (enemyChoice_ >= 4 && enemyChoice_ <= (3+enemy_->getAbilitiesCount())){
            if (enemyAbilities_[enemyChoice_-4]->getMovesCount() == enemyAbilities_[enemyChoice_-4]->getMaxMovesCount()){
                break;
            }
    }
    }
