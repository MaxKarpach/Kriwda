...
AbilitiesSystem::AbilitiesSystem(Player& player, std::vector<Ability>& abilities, Renderer& renderer):
    player_(player), abilities_(abilities), renderer_(renderer){}

template<typename T> T* AbilitiesSystem::findById(int id, std::vector<T>& vec) {
  for (auto& obj : vec) {
    if (obj.getId() == id) {
      return &obj;
    }
...
void AbilitiesSystem::changeAbilities() {
  ...
  while (true) {
    ...
    for (int i = 0; i < player_.getAbilities().size(); i++) {
      Ability* ability = findById<Ability>(player_.getAbilities()[i], abilities_);
      if (ability != nullptr) {
    ...
    for (int i = 0; i < player_.getAbilitiesCount(); i++) {
      Ability* ability = findById<Ability>(player_.getChosenAbilities()[i], abilities_);
      if (ability != nullptr) {
        renderer_.printText(i + 1);
        renderer_.printText(": ");
        renderer_.printEndlineText(ability->getName());
    ...
    if (inputAbilityIndex < 1 || inputAbilityIndex > player_.getAbilities().size()) {
    ...
    if (alreadyChosen) {
      renderer_.printEndlineText("Эта способность уже выбрана. Выберите другую.");
      continue;
    }
    renderer_.printEndlineText("Введите номер выбранной способности, которую хотите заменить: ");
